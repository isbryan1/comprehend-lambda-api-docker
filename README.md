...
```bash
comprehend-lambda-api-docker$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
comprehend-lambda-api-docker$ sam local start-api
comprehend-lambda-api-docker$ curl http://localhost:3000/
```
...
```bash
comprehend-lambda-api-docker$ sam logs -n HelloWorldFunction --stack-name comprehend-lambda-api-docker --tail
```
